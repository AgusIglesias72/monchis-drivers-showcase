...
/**
 * Botón Contactar que abre un dropdown con dos acciones:
...
export function ContactButton({
  ...
  const handleWhatsApp = (e: React.MouseEvent) => {
    ...
    // Si el número viene en formato local (0xxx o sin 595), normalizamos a 595.
    let waPhone = cleanPhone
    if (waPhone.startsWith('0')) waPhone = waPhone.substring(1)
    if (!waPhone.startsWith('595')) waPhone = `595${waPhone}`
    ...
    <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
      <AlertDialogContent onClick={(e) => e.stopPropagation()}>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={confirmRegister} disabled={isRegistering}>
            {isRegistering ? (
              <>
